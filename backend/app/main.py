...
@app.post("/api/mal-kabul", response_model=schemas.MalKabulCikti)
def mal_kabul_olustur(
    veri: schemas.MalKabulOlustur,
    db: Session = Depends(get_db),
    kullanici: models.Kullanici = Depends(rol_gerekli("personel")),
):
    kayit = crud.mal_kabul_olustur(db, veri, kullanici)
    crud.denetim_kaydet(
        db, kullanici.kullanici_adi, "mal_kabul_olustur", "mal_kabul", kayit.id,
        yeni_deger={"tedarikci": veri.tedarikci, "kalem_sayisi": len(veri.kalemler)},
    )
    return kayit


@app.get("/api/mal-kabul", response_model=List[schemas.MalKabulCikti])
def mal_kabul_listele(
    limit: int = 100,
    db: Session = Depends(get_db),
    _: models.Kullanici = Depends(gecerli_kullanici),
):
    return crud.mal_kabul_listele(db, limit)


@app.get("/api/mal-kabul/{mal_kabul_id}", response_model=schemas.MalKabulCikti)
def mal_kabul_detay(
    mal_kabul_id: int,
    db: Session = Depends(get_db),
    _: models.Kullanici = Depends(gecerli_kullanici),
):
    return crud.mal_kabul_getir(db, mal_kabul_id)


# ---------------- Sevkiyat ----------------

@app.post("/api/sevkiyat", response_model=schemas.SevkiyatCikti)
def sevkiyat_olustur(
    veri: schemas.SevkiyatOlustur,
    db: Session = Depends(get_db),
    kullanici: models.Kullanici = Depends(rol_gerekli("personel")),
):
    kayit = crud.sevkiyat_olustur(db, veri, kullanici)
    crud.denetim_kaydet(
        db, kullanici.kullanici_adi, "sevkiyat_olustur", "sevkiyat", kayit.id,
        yeni_deger={"musteri": veri.musteri, "kalem_sayisi": len(veri.kalemler)},
    )
    return kayit


@app.get("/api/sevkiyat", response_model=List[schemas.SevkiyatCikti])
def sevkiyat_listele(
    limit: int = 100,
    db: Session = Depends(get_db),
    _: models.Kullanici = Depends(gecerli_kullanici),
):
    return crud.sevkiyat_listele(db, limit)


@app.get("/api/sevkiyat/{sevkiyat_id}", response_model=schemas.SevkiyatCikti)
def sevkiyat_detay(
    sevkiyat_id: int,
    db: Session = Depends(get_db),
    _: models.Kullanici = Depends(gecerli_kullanici),
):
    return crud.sevkiyat_getir(db, sevkiyat_id)
