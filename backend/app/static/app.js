...
// Türkçe klavye/tarayıcı ayarında ondalık ayırıcı olarak virgül kullanılır
// (örn: "5,5"). Tarayıcının number kutuları bunu kabul etmez ve sessizce
// boş değer döner. Bu yüzden metin kutusu kullanıp virgülü noktaya
// çevirerek sayıya dönüştürüyoruz.
function sayiAyristir(metin) {
...
async function hareketYap(tip) {
  ...
  const miktar = sayiAyristir(document.getElementById("hareketMiktar").value);
...
async function urunEkle() {
  ...
  const miktar = sayiAyristir(document.getElementById("yeniMiktar").value) || 0;
...
function mkSatirEkle() {
  ...
  satir.innerHTML = `
    ...
    <input type="text" placeholder="Gelen miktar" class="mk-miktar" inputmode="decimal">
...
async function mkKaydet() {
  ...
  const kalemler = satirlar
    .map((s) => ({
      ...
      gelen_miktar: sayiAyristir(s.querySelector(".mk-miktar").value),
...
function svSatirEkle() {
  ...
  satir.innerHTML = `
    ...
    <input type="text" placeholder="Miktar" class="sv-miktar" inputmode="decimal">
    <button class="kalem-sil-btn" onclick="this.parentElement.remove()">✕</button>
  `;
  kutu.appendChild(satir);
}
...
async function svKaydet() {
  ...
  const kalemler = satirlar
    .map((s) => ({
      ...
      miktar: sayiAyristir(s.querySelector(".sv-miktar").value),
    }))
